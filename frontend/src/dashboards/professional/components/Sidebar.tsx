...
// Displays navigation links and professional info
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const navItems: NavItem[] = [
    ...
    {
      name: 'Approved Jobs',
      path: '/professional/approved-jobs',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      path: '/professional/patient-records',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">
                {professional?.specialization?.[0] || 'Healthcare Provider'}
              </p>
            </div>
          </div>
        </div>
            ...
            {navItems.map((item) => (
              <li key={item.path}>
                ...
                >
                  <span
                    className={
                      isActiveRoute(item.path) ? 'text-blue-700' : 'text-gray-500'
                    }
                  >
                    {item.icon}
                  </span>
