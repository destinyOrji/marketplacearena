...
const AllEarnings: React.FC = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const all: any[] = [];
      await Promise.allSettled(profs.slice(0, 30).map(async (p: any) => {
        const eRes = await axios.get(`${API}/admin/professionals/${p.id}/earnings`, h());
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[['Total Platform Revenue', `₦${total.toLocaleString()}`, 'bg-blue-600'], ['Net to Professionals', `₦${net.toLocaleString()}`, 'bg-green-600'], ['Platform Fees', `₦${(total - net).toLocaleString()}`, 'bg-orange-600']].map(([label, value, color]) => (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? <div className="flex justify-center items-center h-48"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600" /></div>
          ...
          : <table className="min-w-full divide-y divide-gray-100">
            <thead className="bg-gray-50"><tr>{['Professional','Type','Sessions','Total','Platform Fee','Net'].map(h => <th key={h} className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-gray-50">
              {rows.map((r, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  <td className="px-6 py-4"><button onClick={() => navigate(`/admin/professionals/${r.id}/earnings`)} className="text-blue-600 hover:underline text-sm font-medium">{r.name}</button></td>
