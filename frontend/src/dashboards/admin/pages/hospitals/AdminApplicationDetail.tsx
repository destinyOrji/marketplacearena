 ...
 * Identical layout to the Hospital ApplicationDetail page.
 * Uses admin token + admin API endpoint.
 */
...
import {
  ...
  FiUser, FiMail, FiPhone, FiFileText, FiExternalLink,
...
// ── helpers ───────────────────────────────────────────────────────────────────
const fullUrl = (p: string) => (!p ? '' : p.startsWith('http') ? p : `${BASE_URL}${p.startsWith('/') ? '' : '/'}${p}`);
const isImage = (u: string) => /\.(jpe?g|png|gif|webp|svg|bmp)(\?.*)?$/i.test(u);
const docLabel = (u: string, fb: string) => { try { return decodeURIComponent(u.split('/').pop()?.split('?')[0] || '') || fb; } catch { return fb; } };
    ...
    <div className="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
      {img ? (
        <a href={href} target="_blank" rel="noopener noreferrer">
          <img src={href} alt={label} className="w-full h-48 object-cover bg-gray-100"
      ...
      <div className="p-3">
        {tag && <span className="text-xs font-semibold uppercase tracking-wide text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{tag}</span>}
...
const AdminApplicationDetail: React.FC = () => {
  // Route: /admin/hospitals/all-applications/:hospitalId/:applicationId
  const { hospitalId, applicationId } = useParams<{ hospitalId: string; applicationId: string }>();
  const navigate = useNavigate();
  const [application, setApplication] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  const getToken = () => authService.getAccessToken() || localStorage.getItem('adminToken') || '';

  useEffect(() => { load(); }, [applicationId, hospitalId]);

  const load = async () => {
    try {
      setLoading(true);
      // Admin API: GET /admin/hospitals/:hospitalId/applications
      // We fetch all and find the one — or use a direct endpoint if available
      const res = await fetch(`${API_URL}/admin/hospitals/${hospitalId}/applications`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.message || 'Failed to load');
      const list: any[] = json.data || [];
      const found = list.find((a: any) => (a._id || a.id) === applicationId);
      if (!found) throw new Error('Application not found');
      setApplication(found);
    } catch (err: any) {
      toast.error(err.message || 'Failed to load application');
      navigate('/admin/hospitals/all-applications');
    } finally {
      setLoading(false);
    }
  };
  ...
  const profName    = user.firstName ? `${user.firstName} ${user.lastName}`.trim() : 'N/A';
  const profEmail   = user.email  || '';
  const profPhone   = user.phone  || prof.phone || '';
  const profType    = prof.professionalType || '';
  const profSpec    = prof.specialization   || '';
  const profYears   = prof.yearsOfExperience ?? 0;
  const profLicense = prof.licenseNumber    || '';
  ...
  const reviewNotes = app.reviewNotes || '';
  ...
  (Array.isArray(prof.certifications) ? prof.certifications : []).forEach((c: any, i: number) => {
    if (c.certificateUrl) docs.push({ url: c.certificateUrl, label: c.name || `Certificate ${i + 1}`, tag: 'Certificate' });
  });
  (Array.isArray(app.attachments) ? app.attachments : []).forEach((url: string, i: number) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <button onClick={() => navigate('/admin/hospitals/all-applications')}
          className="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900">
          <FiArrowLeft className="h-4 w-4" /> Back to All Applications
        </button>
        {/* Admin reads only — no accept/reject buttons */}
        ...
        <div className="flex items-start gap-5 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5 text-sm text-gray-600">
              {profEmail && <span className="flex items-center gap-1.5"><FiMail className="h-3.5 w-3.5 text-gray-400" />{profEmail}</span>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {profSkills.map((s: string, i: number) => (
                  <span key={i} className="px-2.5 py-1 text-xs font-medium bg-blue-50 text-blue-700 rounded-full border border-blue-100">{s}</span>
                ))}
              </div>
            </div>
          )}
        ...
        {/* Right: docs, cover letter, qualifications */}
        <div className="lg:col-span-2 space-y-6">
          ...
          {coverLetter && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed bg-gray-50 rounded-xl p-4 border border-gray-100">{coverLetter}</p>
            </div>
          )}
