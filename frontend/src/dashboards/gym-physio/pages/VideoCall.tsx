// Gym-Physio Video Call — WhatsApp-style UI via shared VideoCallScreen
...
const VideoCall: React.FC = () => {
        ...
        if (apt) {
          setAppointment({
            ...
            patientName: apt.patient?.name
              || (apt.client?.user ? `${apt.client.user.firstName} ${apt.client.user.lastName}`.trim() : 'Client'),
  ...
  if (started && appointmentId) {
    const clientName = appointment?.patientName || 'Client';
    return (
      <VideoCallScreen
        appointmentId={appointmentId}
        displayName="Provider"
        remoteDisplayName={clientName}
        remoteInitial={clientName.charAt(0).toUpperCase()}
        serviceTitle={appointment?.serviceTitle}
        onEnd={() => navigate('/gym-physio/record-appointment')}
      />
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="text-center max-w-sm w-full">
        <div className="w-24 h-24 rounded-full bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center mx-auto mb-5 shadow-xl ring-4 ring-orange-500/30">
          <span className="text-white font-bold text-4xl">{initial}</span>
        </div>
        <h2 className="text-xl font-bold text-white mb-1">{clientName}</h2>
